    ...
    ILogger<TransactionsController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] CreateTransactionDto dto, CancellationToken ct)
    {
        ...
        {
            logger.LogWarning("Validation error creating transaction: {Message}", ex.Message);
            return BadRequest(ApiResponse<string>.Fail(ex.Message, correlationId: CorrelationId));
        }
    }
    ...
    public async Task<IActionResult> GetById(Guid id, CancellationToken ct)
    {
        ...
        return result is null
            ? NotFound(ApiResponse<string>.Fail($"Transaction {id} not found", correlationId: CorrelationId))
    ...
    {
        var result = await transactionService.GetPagedAsync(page, pageSize, startDate, endDate, type, ct);
        return Ok(ApiResponse<PagedResponse<TransactionDto>>.Ok(result, correlationId: CorrelationId));
    }
    ...
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateTransactionDto dto, CancellationToken ct)
    {
        ...
        {
            return NotFound(ApiResponse<string>.Fail(ex.Message, correlationId: CorrelationId));
        }
        catch (ArgumentException ex)
        {
            return BadRequest(ApiResponse<string>.Fail(ex.Message, correlationId: CorrelationId));
